...
export const CMMS_Page: React.FC = () => {
  const [dataList, setDataList] = useState<any[]>([]);
  const [metrics, setMetrics] = useState<any | null>(null);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeTab, setActiveTab] = useState('equipmentasset');
  ...
  const loadData = async (modelName: string) => {
    try {
      setLoading(true);
      const res = await api.get('/cmms/' + modelName);
      if (res.data?.success) {
        setDataList(res.data.data.data);
      }
      const mRes = await api.get('/cmms/' + modelName + '/metrics');
      if (mRes.data?.success) {
        setMetrics(mRes.data.data);
      }
    } catch (err) {
      console.error('Failed to load Plant Maintenance & Reliability Engineering', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex border-b border-slate-800 gap-2 overflow-x-auto pb-1">
        ["EquipmentAsset","PreventivePlan","BreakdownLog","WorkPermit","SpareReservation","LubricationSchedule","VibrationAnalysis"].map((m) => (
          <button
            key={m}
