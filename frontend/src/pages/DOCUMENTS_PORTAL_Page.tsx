...
export const DOCUMENTS_PORTAL_Page: React.FC = () => {
  const [dataList, setDataList] = useState<any[]>([]);
  const [metrics, setMetrics] = useState<any | null>(null);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeTab, setActiveTab] = useState('documenttemplate');
  ...
  const loadData = async (modelName: string) => {
    try {
      setLoading(true);
      const res = await api.get('/documents_portal/' + modelName);
      if (res.data?.success) {
        setDataList(res.data.data.data);
      }
      const mRes = await api.get('/documents_portal/' + modelName + '/metrics');
      if (mRes.data?.success) {
        setMetrics(mRes.data.data);
      }
    } catch (err) {
      console.error('Failed to load Document Automation & PDF Vault', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex border-b border-slate-800 gap-2 overflow-x-auto pb-1">
        ["DocumentTemplate","GeneratedPDF","SignatureRecord","VersionHistory","DistributionLog","RetentionPolicy","WatermarkConfig"].map((m) => (
          <button
            key={m}
