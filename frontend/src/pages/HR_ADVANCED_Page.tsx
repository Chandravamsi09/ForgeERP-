...
export const HR_ADVANCED_Page: React.FC = () => {
  const [dataList, setDataList] = useState<any[]>([]);
  const [metrics, setMetrics] = useState<any | null>(null);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeTab, setActiveTab] = useState('shiftroster');
  ...
  const loadData = async (modelName: string) => {
    try {
      setLoading(true);
      const res = await api.get('/hr_advanced/' + modelName);
      if (res.data?.success) {
        setDataList(res.data.data.data);
      }
      const mRes = await api.get('/hr_advanced/' + modelName + '/metrics');
      if (mRes.data?.success) {
        setMetrics(mRes.data.data);
      }
    } catch (err) {
      console.error('Failed to load Human Capital & Advanced Payroll', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-100 flex items-center gap-3">
            ...
            Human Capital & Advanced Payroll
          </h1>
      ...
      <div className="flex border-b border-slate-800 gap-2 overflow-x-auto pb-1">
        ["ShiftRoster","SkillMatrix","OvertimePolicy","StatutoryTaxSlab","BenefitDeduction","PerformanceReview","TrainingRecord"].map((m) => (
          <button
            key={m}
